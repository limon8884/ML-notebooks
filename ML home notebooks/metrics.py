...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for i in range(len(reference)):
        numerator += len(set(reference[i].possible + reference[i].sure) & set(predicted[i]))  
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    denominator = 0
    for i in range(len(reference)):
        numerator += len(set(reference[i].possible + reference[i].sure) & set(predicted[i])) + len(set(reference[i].sure) & set(predicted[i])) 
        denominator += len(predicted[i]) + len(reference[i].sure)
